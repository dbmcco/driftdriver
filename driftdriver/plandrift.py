        ...
        findings.append(
            {
                ...
                "recommendation": (
                    f"Add explicit {missing_label} task(s) after `{task_id}` (use `wg add --after {task_id}`, then `wg publish <id> --only`) and keep them on the critical path before completion."
                ),
            }
        )
    ...
    *,
    ...
    for row in review_rows[: max(1, int(max_tasks))]:
        ...
        if result == "created":
            ...
            out["errors"].append(f"{repo_path.name}: could not create {task_id}")
