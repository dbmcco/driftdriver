...
"$ROOT/bin/driftdriver" --dir "$TMPDIR" install --no-ensure-contracts --speedrift-bin "$SPEEDRIFT_DUMMY" --specdrift-bin "$SPECDRIFT_DUMMY" --datadrift-bin "$DATADRIFT_DUMMY" --depsdrift-bin "$DEPSDRIFT_DUMMY" --uxdrift-bin "$UXDRIFT_DUMMY" >/dev/null
...
rm -f "$SPEEDRIFT_MARKER" "$SPECDRIFT_MARKER" "$DATADRIFT_MARKER" "$DEPSDRIFT_MARKER" "$UXDRIFT_MARKER"
...
rm -f "$SPEEDRIFT_MARKER" "$SPECDRIFT_MARKER" "$UXDRIFT_MARKER"
...
"$ROOT/bin/driftdriver" --dir "$TMPDIR2" install --wrapper-mode portable --with-uxdrift --no-ensure-contracts >/dev/null
...
rm -f "$SPEEDRIFT_MARKER_2" "$SPECDRIFT_MARKER_2" "$DATADRIFT_MARKER_2" "$DEPSDRIFT_MARKER_2" "$UXDRIFT_MARKER_2"
./.workgraph/drifts check --task core-task --write-log --create-followups >/dev/null
