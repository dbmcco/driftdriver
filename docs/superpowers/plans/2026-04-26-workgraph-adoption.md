...
**Goal:** Adopt the highest-value upstream `workgraph` changes into our fork in a staged, testable way, starting with graph-level recovery, then the worktree substrate, then the narrow lifecycle cleanup sweep, then the narrow provider-model execution-routing slice extended with agent-level preferred model/provider routing, endpoint-aware native execution, config-layer endpoint command surfaces, and bootstrap/init endpoint seeding, followed by the first session-runtime precursor of persisted daemon reload overrides, primary-coordinator `service set-executor` compatibility, surfaced coordinator runtime errors, safer pause-by-default `wg kill` semantics, the primary-coordinator subprocess runtime behind `spawn-task` plus `claude-handler`, and the single-owner coordinator session lock on the current global chat store, rather than a blind repo-wide convergence move.

**Architecture:** Treat the current adopted `workgraph` line as canonical until upstream replacements are proven better under stronger contracts. Land changes by tranche: first graph-level recovery, then minimal worktree-isolation substrate, then the dependent coordinator cleanup sweep, then the narrow execution-routing slice, then the endpoint-aware native execution extension of that slice, then the config-layer endpoint command extension, then the bootstrap/init endpoint extension, then persisted daemon override state as the first session-runtime precursor, then the safer pause-by-default `wg kill` semantics, then the primary-coordinator subprocess runtime over the current global chat store, then the single-owner coordinator session lock over that same chat substrate, then broader handler/runtime adoption. Use upstream commit lift or cherry-pick where clean, preserve local fork behavior by default, and strengthen `driftdriver` compatibility checks after each landed tranche.
...
- `wg config --set-endpoint` persists role-level endpoint bindings and exposes them in
  `wg config --models`
- `wg init --model/--endpoint` seeds the same endpoint/model intent during bootstrap
